...
async function askAI(userId, userMessage) {
  ...
  if (technical) {
    // Groq is primary for code/Roblox, Gemini as fallback
    console.log('🔧 Technical task → trying Groq');
    try {
      reply = await askGroqDirect(history);
      console.log('✅ Groq handled it');
    } catch (err) {
      console.warn('⚠️ Groq failed, falling back to Gemini...', err.message);
      try {
        reply = await askGeminiDirect(userMessage, history);
        console.log('✅ Gemini handled it (fallback)');
      } catch (err2) {
        throw new Error('both ai providers are down rn, try again in a bit');
      }
    }
  } else {
    // Gemini is primary for general chat, Groq as fallback
    console.log('💬 General chat → trying Gemini');
    try {
      reply = await askGeminiDirect(userMessage, history);
      console.log('✅ Gemini handled it');
    } catch (err) {
      console.warn('⚠️ Gemini failed, falling back to Groq...', err.message);
      try {
        reply = await askGroqDirect(history);
        console.log('✅ Groq handled it (fallback)');
      } catch (err2) {
        throw new Error('both ai providers are down rn, try again in a bit');
      }
    }
